...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 sidebar-gradient text-white flex flex-col z-50 transition-transform duration-300 ${mobileOpen ? "translate-x-0 mobile-sidebar-enter" : "-translate-x-full"} md:translate-x-0`}>
        ...
        <nav className="flex-1 py-4 px-3 overflow-y-auto">
          <ul className="space-y-1">
            {menuItems.map((item) => {
              ...
              return (
                <li key={item.href}>
                  <Link href={item.href} className={`flex items-center gap-3 px-3 sm:px-4 py-2.5 rounded-xl transition-all text-[13px] sm:text-sm sidebar-item-hover ${isActive ? "sidebar-item-active bg-white/[0.08] text-white font-medium" : "text-gray-400 hover:text-gray-200"}`}>
