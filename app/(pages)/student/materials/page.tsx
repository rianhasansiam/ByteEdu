      ...
      {subjects.length > 0 && (
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          <button onClick={() => setFilter("")}
            className={`px-4 py-2 rounded-lg text-sm transition-colors whitespace-nowrap ${!filter ? "bg-indigo-600 text-white" : "bg-white border text-gray-700 hover:bg-gray-50"}`}>
            All
          </button>
          {subjects.map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-4 py-2 rounded-lg text-sm transition-colors whitespace-nowrap ${filter === s ? "bg-indigo-600 text-white" : "bg-white border text-gray-700 hover:bg-gray-50"}`}>
              {s}
            </button>
          ))}
        </div>
            ...
            <div key={m.id} className="bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-indigo-600">{m.subjectName}</p>
              ...
              {m.fileUrl && (
                <a href={m.fileUrl} target="_blank" rel="noopener noreferrer"
                  className="mt-3 inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-sm hover:bg-indigo-100 transition-colors">
